...
class RouteNet(tf.keras.Model):
    ...
    def build(self, input_shape=None):
        ...
        self.path_update = tf.keras.layers.GRUCell(
            self.hparams.path_state_dim, name="path_update"
        )
...
def delay_model_fn(
    ...
    if mode == tf.estimator.ModeKeys.PREDICT:
        return tf.estimator.EstimatorSpec(
            mode, predictions={"delay": delay_prediction, "jitter": jitter_prediction}
        )
    ...
    with tf.name_scope("heteroscedastic_loss"):
        ...
        nll = (
            n * y["jitter"] / _2sigma
            + n * tf.math.squared_difference(y["delay"], loc) / _2sigma
            + n * tf.math.log(scale)
        )
    ...
    if mode == tf.estimator.ModeKeys.EVAL:
        return tf.estimator.EstimatorSpec(
            ...
            eval_metric_ops={
                ...
                "mae/delay": tf.metrics.mean_absolute_error(
                    labels["delay"], delay_prediction
                ),
                "mae/jitter": tf.metrics.mean_absolute_error(
                    labels["jitter"], jitter_prediction
                ),
                "rho/delay": tf.contrib.metrics.streaming_pearson_correlation(
                    labels=labels["delay"], predictions=delay_prediction
                ),
                "rho/jitter": tf.contrib.metrics.streaming_pearson_correlation(
                    labels=labels["jitter"], predictions=jitter_prediction
                ),
            },
        )
    ...
    decayed_lr = tf.train.exponential_decay(
        params.learning_rate, tf.train.get_global_step(), 50000, 0.9, staircase=True
    )
...
def drop_model_fn(
    ...
    if mode == tf.estimator.ModeKeys.PREDICT:
        return tf.estimator.EstimatorSpec(
            mode, predictions={"drops": predictions, "logits": logits}
        )
    ...
    if mode == tf.estimator.ModeKeys.EVAL:
        return tf.estimator.EstimatorSpec(
            ...
            eval_metric_ops={
                ...
                "rho/drops": tf.contrib.metrics.streaming_pearson_correlation(
                    labels=loss_ratio, predictions=predictions
                ),
            },
        )
        ...
        tensors = (
            {
                ...
                "links": tf.concat(
                    [v["links"] + m for v, m in zip(vs, links_cummax)], axis=0
                ),
                "paths": tf.concat(
                    [v["paths"] + m for v, m in zip(vs, paths_cummax)], axis=0
                ),
...
def train(args):
    ...
    best_exporter = tf.estimator.BestExporter(
        serving_input_receiver_fn=serving_input_receiver_fn, exports_to_keep=2
    )
    ...
    train_spec = tf.estimator.TrainSpec(
        input_fn=lambda: tfrecord_input_fn(
            args.train, hparams, shuffle_buf=args.shuffle_buf, target=args.target
        ),
        max_steps=args.train_steps,
    )
    eval_spec = tf.estimator.EvalSpec(
        input_fn=lambda: tfrecord_input_fn(
            args.evaluation, hparams, shuffle_buf=None, target=args.target
        ),
